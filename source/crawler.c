...
void LoadAssets()
{
	...
	axeTexture = LoadTexture("textures/axe.png");
...
extern GAME_ON_UPDATE(OnUpdate)
{
	...
	else
	{
		...
		for (u64 i = 0; i < state.enemiesSize; ++i)
		{
			...
			if (V2Distance(e->pos, frog->pos) < 10.0f)
			{
				...
				if (e != trapdoor)
				{
					...
					{
						f32 sec = Seconds(1.5f);
						if (e->item != NULL)
							sec = Seconds(e->item->cooldownTime);
						if (currentTime > frog->lastTimeHurt + Seconds(sec) && V2Distance(e->pos, frog->pos) <= e->scale.x - 0.1f)
						{
		...
		if (loadNextLevel && transitionOpacity > 0.9f)
		{
			UnloadMap(true);
			u64 i = 0;
